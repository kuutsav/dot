---
name: dot-tmux-test
description: E2E testing of dot using tmux sessions; IMPORTANT: only trigger this skill when user asks for e2e testing of dot
---

# Dot Tmux E2E Testing

End-to-end testing of dot using tmux sessions to programmatically control the TUI application.

## Why Tmux?

Dot is a TUI (Textual-based) app. Running tests programmatically is hard. Tmux provides:
- `tmux new-session` - isolate test environment
- `tmux send-keys` - send keyboard input
- `tmux capture-pane` - capture output
- `tmux has-session` - check if dot is running

## Test Design Philosophy

- **Deterministic**: Shell scripts create reproducible test environments
- **Separation of concerns**: Shell script runs steps and captures output; dot (AI) evaluates results
...
```bash
# Run all e2e tests
bash ~/.dot/skills/dot-tmux-test/run-e2e-tests.sh

# After running, dot (the AI) reads the output files and evaluates results
# Output files: /tmp/dot-test-*.txt
```
...
Creates the same deterministic test project structure each time at `/tmp/dot-test-project/`:

```bash
bash ~/.dot/skills/dot-tmux-test/setup-test-project.sh
```

Test project structure:
```
/tmp/dot-test-project/
...
# After running, the AI (dot) reads output files and provides evaluation:
# - /tmp/dot-test-1-commands.txt       (/ slash commands list)
# - /tmp/dot-test-2-at-trigger.txt     (@ file picker test)
# - /tmp/dot-test-3-model.txt          (/model selector test)
# - /tmp/dot-test-4-new.txt            (/new conversation test)
# - /tmp/dot-test-5-tab-unique.txt     (Tab completion unique match)
# - /tmp/dot-test-6-tab-multiple.txt   (Tab completion floating list)
# - /tmp/dot-test-7-tab-home.txt       (Tab completion home directory)
# - /tmp/dot-test-8-tab-select.txt     (Tab completion select from list)
# - /tmp/dot-test-9-tools.txt          (tool execution test)
# - /tmp/dot-test-10-session.txt       (/session stats test)
# - /tmp/dot-test-11-resume.txt        (/resume session list test)
# - /tmp/dot-test-files.txt            (file system state)
# - /tmp/dot-test-test1-content.txt    (test1.txt content)
```
...
SESSION_NAME="dot-test"  # Tmux session name
TEST_DIR="/tmp/dot-test-project"  # Test project directory for tool execution
DOT_DIR="$HOME/Developer/personal/dot"  # Dot repo directory (for tab completion tests)
```
...
## Test Evaluation (by Dot)

After running the test script, dot evaluates the results by reading the output files:
...
- `test1.txt` created in /tmp/dot-test-project/
- `test1.txt` contains "world" (not "hello")
- Directory listing shows files

### Tabular Report

Dot provides a tabular summary after reading all output files, showing:
- Test name
- Status (PASS/FAIL)
- Description
...
# Output files remain for dot to evaluate (/tmp/dot-test-*.txt)
# Manual cleanup if needed:
tmux kill-session -t dot-test 2>/dev/null
rm -rf /tmp/dot-test-project
...
- Tab completion tests run from dot repo to test on known directory structure
...
- Output files saved to `/tmp/dot-test-*.txt` for debugging
