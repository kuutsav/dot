...
def build_system_prompt(cwd: str, context: Context | None = None) -> str:
    ...
    prompt = dot_config.llm.system_prompt
...
class Agent:
            ...
            while turn < self.config.max_turns:
                ...
                async for compaction_event in self._check_compaction(
                    ...
                    if isinstance(compaction_event, CompactionEndEvent):
                        did_compact = True
                if did_compact:
                    if dot_config.compaction.on_overflow == "pause":
                        break
    ...
    async def _check_compaction(
        ...
        context_window = self.config.context_window or dot_config.compaction.default_context_window
        max_output = self.config.max_output_tokens or self.provider.config.max_tokens
        buffer_tokens = dot_config.compaction.buffer_tokens
        ...
        try:
            ...
            if dot_config.compaction.on_overflow == "continue":
