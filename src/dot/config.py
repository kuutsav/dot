...
CONFIG_DIR_NAME: str = ".dot"

OnOverflowMode = Literal["continue", "pause"]


def _load_default_config_text() -> str:
    return resources.files("dot.defaults").joinpath("config.toml").read_text(encoding="utf-8")


_DEFAULT_CONFIG_DATA = tomllib.loads(_load_default_config_text())

_config_var: ContextVar["Config | None"] = ContextVar("dot_config", default=None)
