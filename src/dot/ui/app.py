...
from dot import config, consume_config_warnings, update_available_binaries
from dot.tools_manager import ensure_tools
...
def _default_base_url_for_api(api_type: ApiType) -> str | None:
    if api_type == ApiType.OPENAI_COMPLETIONS:
        return os.environ.get("DOT_BASE_URL", "https://api.z.ai/api/coding/paas/v4")
    return None


class DotApp(CommandsMixin, SessionUIMixin, App[None]):
    CSS = STYLES
    TITLE = "dot"
    VERSION = VERSION
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Dot TUI")
    ...
    messages: list[str] = []
    if app._session:
        messages.append(f"dot -r {app._session.id}")
